...
export async function runIndexCommand(options: IndexCommandOptions): Promise<void> {
  ...
  const { observer, resolvedTraceFilePath } = await createCliObserver(options.traceFilePath);
  ...
  try {
    ...
    const resolvedIndexFilePath = await writeIndexSnapshot(options.indexFilePath, snapshot);
