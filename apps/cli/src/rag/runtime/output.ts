...
export function buildAnswer(input: { chunks: Chunk[]; query: string }): string {
  ...
  return [
    ...
    ...input.chunks.map((chunk, index) => {
      const title =
        typeof chunk.metadata?.documentTitle === 'string' ? chunk.metadata.documentTitle : chunk.id;
...
export function printAskSummary(input: {
  indexingResult: Pick<IndexingResult, 'documentsIndexed' | 'chunksTotal' | 'vectorsTotal'>;
  ...
  for (const [index, candidate] of input.retrievalResult.candidates.entries()) {
    const title =
      typeof candidate.chunk.metadata?.documentTitle === 'string'
        ...
        : candidate.chunk.id;
    const score = typeof candidate.score === 'number' ? candidate.score.toFixed(6) : 'n/a';
    ...
    if (input.debug) {
      ...
      console.log(`  metadata: ${JSON.stringify(candidate.chunk.metadata ?? {})}`);
    }
  }

  if (input.retrievalResult.retrievalMetadata) {
    console.log(`retrievalMetadata: ${JSON.stringify(input.retrievalResult.retrievalMetadata)}`);
  }
