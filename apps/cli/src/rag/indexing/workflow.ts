...
import { LangChainDirectoryLoaderAdapter, UnknownHandling } from '@monai-ragsdk/adapters';
import type { Chunk, Document, Vector } from '@monai-ragsdk/core';
import { BasicMetadataExtractor, SimpleChunker, runIndexing } from '@monai-ragsdk/indexing';
...
import type { IndexBuildOptions, IndexSnapshot, IndexedChunkMap } from '../../types.js';
...
export async function runLocalIndexing(input: {
  ...
  const resolvedDirectoryPath = path.resolve(process.cwd(), input.options.directoryPath);

  const loader = new LangChainDirectoryLoaderAdapter({
    directoryPath: resolvedDirectoryPath,
    loaders: Object.fromEntries(
      input.options.extensions.map((ext) => [ext, (filePath: string) => new TextLoader(filePath)]),
    ),
    recursive: true,
  ...
  const headerPathTransformer: DocumentTransformer = {
    async transform(document: Document): Promise<Document> {
      ...
      const relativePath =
        path.relative(resolvedDirectoryPath, sourcePath) || path.basename(sourcePath);
  ...
  return {
    ...
    indexingResult,
    chunks: memoryStore
      ? mergeIndexedChunks(input.previousChunks ?? [], chunkMap, memoryStore.getAll())
