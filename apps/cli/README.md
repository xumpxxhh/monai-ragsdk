...
`apps/cli` 是一个独立的业务接入项目，用于直接消费 MonAI RAG SDK，验证本地资料目录的索引与查询链路。
...
```bash
pnpm --filter @monai-ragsdk/cli cli:dev -- index --dir ./docs --indexFile ./apps/cli/.artifacts/index-snapshot.json
```
...
```bash
pnpm --filter @monai-ragsdk/cli cli:dev -- runtime answer --indexFile ./apps/cli/.artifacts/index-snapshot.json --query "什么是 runtime" --debug
```

纯 retrieval 调试：

```bash
pnpm --filter @monai-ragsdk/cli cli:dev -- runtime retrieval --indexFile ./apps/cli/.artifacts/index-snapshot.json --query "什么是 runtime" --debug
```
...
- `--indexFile`：索引快照文件路径，默认 `./apps/cli/.artifacts/index-snapshot.json`
