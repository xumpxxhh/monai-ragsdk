...
import type { RetrievalRequest, RuntimeResult } from '../../packages/runtime/src/index.ts';
...
export async function runRuntimeAdaptersQueryScenario(): Promise<RuntimeAdaptersQueryScenarioResult> {
  const { createLangChainBaseRetrieverRuntimeAdapter, createLangChainChatModelRuntimeGenerator } =
  ...
  } as unknown as Parameters<typeof createLangChainBaseRetrieverRuntimeAdapter>[0];
  ...
  } as unknown as Parameters<typeof createLangChainChatModelRuntimeGenerator>[0];
  const runtime = createDefaultRuntime({
    ...
    retriever: createLangChainBaseRetrieverRuntimeAdapter(retrieverAdapterOptions),
    generator: createLangChainChatModelRuntimeGenerator(generatorAdapterOptions),
  });
