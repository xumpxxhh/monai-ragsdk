...
export const EXAMPLE_REGISTRY: Record<string, ExampleEntry> = {
  observability: {
    id: "observability",
    description:
      "Observability 对照：前后策略 + FanOut，重点查看 trace 事件链路",
    run: runObservabilityExample,
  },
  "basic-stream": {
    id: "basic-stream",
    description:
      "默认栈闭环：索引 → pgvector → 流式生成（无策略，含 observer）",
    run: runBasicStreamExample,
  },
  "pre-retrieval": {
    id: "pre-retrieval",
    description:
      "Pre-retrieval：Rewrite + Multi-Query + FanOut（含 observer）",
    run: runPreRetrievalExample,
  },
  "post-retrieval": {
    id: "post-retrieval",
    description:
      "Post-retrieval：Rerank + Compression + Lost-in-the-Middle（含 observer）",
...
export async function runExampleById(id: string): Promise<void> {
  ...
  if (!entry) {
    throw new Error(
      `未知示例 "${id}"；可用：${ALL_EXAMPLE_IDS.join(", ")}, all, list`,
    );
  }
