...
function mergeObservers(observers: RAGObserver[]): RAGObserver {
  return {
    async onEvent(event) {
      await Promise.all(
        observers.map((item) => item.onEvent?.(event)).filter(Boolean),
      );
    },
    async onError(error) {
      await Promise.all(
        observers.map((item) => item.onError?.(error)).filter(Boolean),
      );
    },
    async onTraceEnd(trace) {
      await Promise.all(
        observers.map((item) => item.onTraceEnd?.(trace)).filter(Boolean),
      );
    },
    async flush() {
      await Promise.all(
        observers.map((item) => item.flush?.()).filter(Boolean),
      );
    },
    async shutdown() {
      await Promise.all(
        observers.map((item) => item.shutdown?.()).filter(Boolean),
      );
    },
  };
}
