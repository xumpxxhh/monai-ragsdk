...
function parseAskCommandOptions(
  ...
  return {
    ...
    traceFilePath:
      values.get("traceFile") ?? "./.artifacts/ask-trace.jsonl",
    debug,
  };
}
...
function parseIndexCommandOptions(
  ...
  return {
    ...
    traceFilePath:
      values.get("traceFile") ?? "./.artifacts/index-trace.jsonl",
    debug,
  };
}
...
function parseRuntimeCommandOptions(
  ...
  return {
    ...
    indexFilePath: values.get("indexFile") ?? DEFAULT_INDEX_FILE_PATH,
    traceFilePath:
      values.get("traceFile") ??
      `./.artifacts/runtime-${subcommand}-trace.jsonl`,
    configFilePath: values.get("config"),
    debug,
  };
}

function parseInitCommandOptions(
  values: Map<string, string>,
): InitCommandOptions {
  return {
    command: "init",
