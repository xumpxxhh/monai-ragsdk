...
export function createConsoleObserver(options: ConsoleObserverOptions = {}): RAGObserver {
  ...
  const includeAttributes = options.includeAttributes ?? configuredLevel === 'debug';
  ...
  return {
    ...
    async onError(error) {
      ...
      await invokeObserverSafely(async () => {
        writeConsole('error', formatError(error), includeAttributes ? error.attributes : undefined);
      });
    },
