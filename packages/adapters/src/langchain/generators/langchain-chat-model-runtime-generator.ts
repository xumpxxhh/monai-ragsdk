...
function buildFallbackHumanMessageContent(
  input: LangChainRuntimeGenerationInput,
): string {
  ...
  return [
    "Question:",
    input.request.effectiveQuery.query,
    "",
    "Context:",
    contextText,
  ].join("\n");
...
>(
  options: CreateLangChainChatModelRuntimeGeneratorOptions<
    CallOptions,
    OutputMessageType
  >,
  input: LangChainRuntimeGenerationInput,
...
>(
  ...
  ChatModelInvokeInput<CallOptions>,
  OutputMessageType
> {
  return new LangChainRuntimeGeneratorAdapter<
    ChatModelInvokeInput<CallOptions>,
    OutputMessageType
  >({
    generator: {
