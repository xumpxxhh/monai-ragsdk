...
describe("LangChainMarkdownDirectoryLoader", () => {
  it("loads markdown files recursively and ignores other extensions by default", async () => {
    const rootDirectory = await mkdtemp(
      path.join(os.tmpdir(), "rag-adapters-loader-"),
    );
    ...
    await writeFile(
      path.join(rootDirectory, "nested", "child.markdown"),
      "# Child",
    );
  ...
  it("allows callers to provide their own file extension mapping", async () => {
    const rootDirectory = await mkdtemp(
      path.join(os.tmpdir(), "rag-adapters-loader-custom-"),
    );
    temporaryDirectories.push(rootDirectory);

    await writeFile(
      path.join(rootDirectory, "guide.txt"),
      "custom loader text",
    );
