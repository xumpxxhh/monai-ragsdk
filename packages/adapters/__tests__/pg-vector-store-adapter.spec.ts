...
describe('PgVectorStoreAdapter', () => {
  ...
  it('upserts vectors with pgvector-compatible values', async () => {
    ...
    expect(query.mock.calls[0]?.[0]).toContain('INSERT INTO "public"."rag_vectors"');
  ...
  it('creates schema and table when ensureTable is enabled', async () => {
    ...
    expect(query.mock.calls[0]?.[0]).toBe('CREATE EXTENSION IF NOT EXISTS vector');
    expect(query.mock.calls[1]?.[0]).toBe('CREATE SCHEMA IF NOT EXISTS "public"');
    expect(query.mock.calls[2]?.[0]).toContain('CREATE TABLE IF NOT EXISTS "public"."rag_vectors"');
    expect(query.mock.calls[2]?.[0]).toContain('"embedding" VECTOR(2) NOT NULL');
