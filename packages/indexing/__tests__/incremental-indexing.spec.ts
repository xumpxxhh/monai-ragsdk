...
describe("runIndexing incremental behavior", () => {
  it("skips embed and upsert when fingerprint is unchanged", async () => {
    ...
    const embed = vi.fn(async (chunks) =>
      new MockEmbedder({ dimension: 4 }).embed(chunks),
    );
  ...
  it("replaces when the same source already has multiple fingerprints", async () => {
    ...
    expect(embed).toHaveBeenCalledTimes(1);
    expect(
      store.getAll().every((vector) => vector.metadata?.fingerprint === "fp-1"),
    ).toBe(true);
  });
  ...
  it("replaces previous vectors when fingerprint changes", async () => {
    ...
    expect(store.getAll().every((vector) => vector.metadata?.fingerprint === "fp-2")).toBe(
      true,
    );
    expect(
      store.getAll().some((vector) => firstIds.includes(vector.id) && vector.metadata?.fingerprint === "fp-1"),
    ).toBe(false);
  });
  ...
  it("deletes stale sources that disappeared from the loader", async () => {
    ...
    expect(store.getAll().every((vector) => vector.metadata?.sourceId === "source-a")).toBe(
      true,
    );
  });
