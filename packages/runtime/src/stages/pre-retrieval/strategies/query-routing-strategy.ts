 ...
 */
export function createQueryRoutingStrategy(
  options: QueryRoutingStrategyOptions,
): QueryStrategy {
  ...
  return {
    async apply(
      ...
      if (budget && typeof budget === "object" && !Array.isArray(budget)) {
        ...
        const maxPromptChars = (budget as { maxPromptChars?: unknown })
          .maxPromptChars;
        next.budget = {
          ...(next.budget ?? {}),
          ...(typeof maxChunks === "number" && maxChunks > 0
            ? { maxChunks }
            : {}),
          ...(typeof maxPromptChars === "number" && maxPromptChars > 0
            ? { maxPromptChars }
            : {}),
        };
      }
