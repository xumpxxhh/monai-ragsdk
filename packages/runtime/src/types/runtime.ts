...
export type Runtime = {
  run(input: RuntimeQueryInput, options?: RuntimeRunOptions): Promise<RuntimeResult>;
  /**
   * retrieve-only：只跑 pre-retrieval → retrieval → post-retrieval。
   * 不调用 generator，因此没有 answer；citations 仍按选出的 chunks 生成。
   */
  search(input: RuntimeQueryInput, options?: RuntimeRunOptions): Promise<RuntimeSearchResult>;
