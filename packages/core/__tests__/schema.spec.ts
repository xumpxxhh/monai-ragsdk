...
describe("core schemas", () => {
  ...
  it("rejects a rag response missing citations", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error?.issues.some((issue) => issue.path[0] === "citations"),
    ).toBe(true);
  });
  ...
  it("rejects citations that do not align with chunks", () => {
    ...
    expect(
      result.error?.issues.some(
        (issue) =>
          issue.path[0] === "citations" &&
          (issue.path[1] === 0 || issue.path.length === 1),
      ),
    ).toBe(true);
  });
  ...
  it("rejects ISO-8601 strings for startedAt and endedAt", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error?.issues.some((issue) => issue.path[0] === "startedAt"),
    ).toBe(true);
  });
  ...
  it("rejects endedAt earlier than startedAt", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error?.issues.some((issue) => issue.path[0] === "endedAt"),
    ).toBe(true);
  });
  ...
  it("rejects a non-finite score threshold", () => {
    ...
    expect(
      result.error?.issues.some(
        (issue) => issue.path[0] === "appliedScoreThreshold",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a single string as stage strategies", () => {
    ...
    expect(result.success).toBe(false);
    expect(
      result.error?.issues.some((issue) => issue.path[0] === "strategies"),
    ).toBe(true);
  });
});
